...
def preprocess(mask1, mask2):
    mask=[np.zeros(mask1.shape),np.zeros(mask2.shape)]
    mask[0][np.where(mask1>mask2)]=1.0
    mask[1][np.where(mask1<mask2)]=1.0
...
def LaplacianPyramid(img, leveln):
    ...
    for i in range(leveln - 1):
        ...
        LP.append(img - cv2.resize(cv2.pyrUp(next_img, img.shape[1::-1]),(img.shape[1],img.shape[0])))
        img = next_img
    LP.append(img)
    return LP
...
def blend_pyramid(LPA, LPB, MPA, MPB):
    ...
    for i in range(len(MPA)):
    	#print(LPA[i].shape,MPA[i].shape,LPB[i].shape,MPB[i].shape)
    	blended.append(LPA[i] * MPA[i] + LPB[i] * MPB[i])
    return blended
...
def reconstruct(LS):
    ...
    for lev_img in LS[-2::-1]:
        ...
        img += cv2.resize(lev_img,(img.shape[1],img.shape[0]))
    return img


def multi_band_blending(img1, img2, mask1, mask2, leveln=None):

    # if img1.shape[0] != img2.shape[0]:
    #     print("error: image dimension error")
    #     sys.exit()
        
    mask = preprocess(mask1, mask2)
    ...
    MPA = GaussianPyramid(mask[0].astype(float), leveln)
    MPB = GaussianPyramid(mask[1].astype(float), leveln)
    LPA = LaplacianPyramid(img1.astype(float), leveln)
    LPB = LaplacianPyramid(img2.astype(float), leveln)

    # Blend two Laplacian pyramidspass
    blended = blend_pyramid(LPA, LPB, MPA, MPB)
    ...
    # flag_half = args['half']
    # img1 = cv2.imread(args['first']).astype(float)
    # img2 = cv2.imread(args['second']).astype(float)
    # leveln = args['leveln']
    # mask1=cv2.imread("1w.jpg").astype(float)
    # mask2=cv2.imread("2w.jpg").astype(float)
    # result = multi_band_blending(img1, img2, mask1, mask2, leveln)
